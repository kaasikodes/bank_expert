...
import { useRemoveOwnerFromDeployedWallet } from "./WalletDetails";
...
export const WalletRemoveOwner: React.FC<TProps> = ({ onClose, open, contractAddress }) => {
  ...
  } = useRemoveOwnerFromDeployedWallet({ deployedContractAddress: contractAddress });
  ...
  return (
    <Modal title="Remove Owner from Wallet" style={{ top: 20 }} open={open} onCancel={handleCancel} footer={null}>
      <Skeleton loading={deployedContractLoading} active paragraph={{ rows: 5 }}>
        ...
        >
          <Form.Item label="Owner Address * " name={"owner"} rules={textInputValidationRules}>
